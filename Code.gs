...
function getZapisyWindow() {
  ...
  const values = sheet.getRange(2, 1, 1, 4).getValues()[0]; // A2:D2
...
function jakiLimitDlaZajecia(id_zajecia, zajeciaData) {
  ...
  return zajecieRow ? zajecieRow[7] : 0; // max_limit
}

function jakieMinimumDlaZajecia(id_zajecia, zajeciaData) {
  const zajecieRow = zajeciaData.find(row => row[0] == id_zajecia);
  return zajecieRow ? zajecieRow[8] : 0; // min_limit
}
...
function zapiszDziecko(zapisywanyUczen, lang) {
  ...
  const zapisyDataDlaUcznia = zapisyData.filter(row => row[3]?.toString().toLowerCase() ===
    zapisywanyUczen.uczen?.toString().toLowerCase());
  ...
  if (!noweZajecie.klasy
    .map(k => k.toLowerCase())
    .includes(klasaUczniaStr.toLowerCase())) {
...
function getZajecieById(id_zajecia, zajeciaData) {
  ...
  const klasy = row[6]
    ? row[6].toString().split(',').map(s => s.trim()).filter(Boolean)
    : [];
...
class Activity {
  constructor(id, nazwa, nazwa_ang, dzien, od, do_, klasyStr, max, min, platne) {
    this.id = id;
    this.nazwa = nazwa;
    this.nazwa_ang = nazwa_ang ? nazwa_ang : nazwa;
    this.dzien = dzien;
